...
app.use('/', express.static('client/cliente', { redirect: false }));

//obtener paises
app.get('/flights/paises', async(req, res) => {
    paises = [];
    let archivoOrigen = " ";
    archivoOrigen = './assets/paises.json';
...
app.get('/flights/ciudades', async(req, res) => {
    ciudades = [];
    let archivoCiudades = " ";
    archivoCiudades = './assets/ciudades.json';
...
app.get('*', (res) => {
    res.sendFile(path.resolve('client/cliente/index.html'));
});
